...
class EpisodeBatch(tp.Generic[T]):
    ...
    _next_physics: tp.Optional[T] = None
    _future_physics: tp.Optional[T] = None
...
class ReplayBuffer:
    ...
    def sample(self, batch_size, custom_reward: tp.Optional[tp.Any] = None, with_physics: bool = False) -> EpisodeBatch:
        ...
        phy = self._storage['physics'][ep_idx, step_idx-1]
        next_phy = self._storage['physics'][ep_idx, step_idx]
        ...
        if self._future < 1:
            ...
            future_phy = self._storage['physics'][ep_idx, future_idx-1]
            curr_phy = self._storage['physics'][ep_idx, step_idx-1]
            random_num1 = np.random.rand(batch_size)
            future_obs = np.where((random_num1 < self._p_currgoal / (1. - self._p_randomgoal)).reshape(future_obs.shape[0], *[1] * (len(future_obs.shape) - 1)), curr_obs, future_obs)
            future_phy = np.where((random_num1 < self._p_currgoal / (1. - self._p_randomgoal)).reshape(future_phy.shape[0], *[1] * (len(future_phy.shape) - 1)), curr_phy, future_phy)
            random_obs = self.get_obs('observation', random_ep_idx, random_step_idx - 1)
            random_phy = self._storage['physics'][random_ep_idx, random_step_idx-1]
            random_num2 = np.random.rand(batch_size)
            future_obs = np.where((random_num2 < self._p_randomgoal).reshape(future_obs.shape[0], *[1] * (len(future_obs.shape) - 1)), random_obs, future_obs)
            future_phy = np.where((random_num2 < self._p_randomgoal).reshape(future_phy.shape[0], *[1] * (len(future_phy.shape) - 1)), random_phy, future_phy)
        
        additional = {}
        if with_physics:
            additional["_physics"] = phy
            additional["_next_physics"] = next_phy
            additional["_future_physics"] = future_phy
