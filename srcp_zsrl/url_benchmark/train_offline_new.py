...
from url_benchmark.in_memory_replay_buffer_new import ReplayBuffer
...
class Config:
    ...
    # checkpoint
    snapshot_at: tp.Tuple[int, ...] = (500000,)
...
def _init_eval_meta(workspace, custom_reward: BaseReward = None):
    ...
    while batch_size < num_steps:
        ...
        # print("batch.next_obs", batch.next_obs.shape)
...
class Workspace:
    ...
    def load_checkpoint(self, fp: tp.Union[Path, str], only: tp.Optional[tp.Sequence[str]] = None, exclude: tp.Sequence[str] = (), num_episodes=None, use_pixels=False) -> None:
        ...
        if use_pixels:
            # 增加states
            payload._storage['states'] = payload._storage['observation']
