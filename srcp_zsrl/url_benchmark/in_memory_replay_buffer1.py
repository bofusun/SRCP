#########################################################################
# 额外batch中增加一个next_next_obs，匹配train_offline1与fb_ddpg8,9
#########################################################################
import logging
import typing as tp
import dataclasses
...
class EpisodeBatch(tp.Generic[T]):
    ...
    def unpack(self) -> tp.Tuple[T, T, T, T, T, T]:
        ...
        """
        return (self.obs, self.action, self.reward, self.discount, self.next_obs, self.next_next_obs)
...
class ReplayBuffer:
    ...
    def sample(self, batch_size, custom_reward: tp.Optional[tp.Any] = None, with_physics: bool = False) -> EpisodeBatch:
        ...
        step_idx = np.random.randint(0, eps_lengths-1) + 1
        ...
        next_next_obs = self.get_obs("observation", ep_idx, step_idx+1)
        ...
        return EpisodeBatch(obs=obs, action=action, reward=reward, discount=discount,
                            next_obs=next_obs, next_next_obs=next_next_obs, future_obs=future_obs, meta=meta, **additional)
