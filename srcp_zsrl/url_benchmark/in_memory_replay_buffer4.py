...
class EpisodeBatch(tp.Generic[T]):
    ...
    def unpack(self) -> tp.Tuple[T, T, T, T, T, T, T]:
        ...
        """
        return (self.obs, self.action, self.action_seq, self.reward, self.discount, self.next_obs, self.r_next_obs, self.neg_next_obs)
...
class ReplayBuffer:
    def __init__(
            ...
            frame_stack: int = None, multistep: int = 3,
            window_size: int = 5,
    ) -> None:
    ...
    def sample(self, batch_size, custom_reward: tp.Optional[tp.Any] = None, with_physics: bool = False) -> EpisodeBatch:
        ...
        step_idx = np.random.randint(0, eps_lengths-self._multistep-self._window_size-1) + 1
        ...
        # action_seq = np.concatenate([self._storage['action'][ep_idx, step_idx+i][:, None, :] for i in range(self._multistep)], axis=1)
        action_seq = np.concatenate([self._storage['action'][ep_idx, step_idx+i] for i in range(self._multistep)], axis=1)  # 沿着第 1 轴拼接，得到 (32, 30)
        next_obs = self.get_obs("observation", ep_idx, step_idx)
        r_next_obs = self.get_obs("observation", ep_idx, step_idx + self._multistep - 1)
        ### sample negative pair of r_next_obs from its nearby states
        if np.random.uniform(-1,1) > 0:
            index = (step_idx + self._multistep - 1) + (np.random.randint(self._window_size)+1) 
        else:
            index = (step_idx + self._multistep - 1) - (np.random.randint(self._window_size)+1) 
        neg_next_obs = self.get_obs("observation", ep_idx, index)
        ...
        return EpisodeBatch(obs=obs, action=action, action_seq=action_seq, reward=reward, discount=discount,
                            next_obs=next_obs, r_next_obs=r_next_obs, neg_next_obs=neg_next_obs, future_obs=future_obs, meta=meta, **additional)
