...
class Workspace:
    ...
    def finalize(self, count) -> None:
        ...
        for name in domain_tasks[self.domain]:
            ...
            self.cfg.seed += count*10000000  # for the sake of avoiding similar seeds
            self.eval_env = self._make_env()
        ...
        with (self.work_dir / ("test_rewards"+str(count)+".json")).open("w") as f:
            json.dump(rewards, f)
