...
class SRCPAgentConfig:
    # @package agent
    _target_: str = "url_benchmark.agent.srcp.SRCPAgent"
    name: str = "srcp"
...
cs.store(group="agent", name="srcp", node=SRCPAgentConfig)
...
class SRCPAgent:
    ...
    def update(self, replay_loader: ReplayBuffer, step: int) -> tp.Dict[str, float]:
        ...
        for _ in range(self.cfg.num_sf_updates):
            ...
            metrics.update(self.update_sf(obs=obs.detach(), action=action, discount=discount, next_obs=next_obs.detach(), future_obs=future_obs.detach(), z=z, step=step))
