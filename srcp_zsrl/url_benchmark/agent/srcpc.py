...
from .fb_modules import DiagGaussianActor, ForwardMap, BackwardMap, mlp, OnlineCov
...
class SRCPCAgentConfig:
    # @package agent
    _target_: str = "url_benchmark.agent.srcpc.SRCPCAgent"
    name: str = "srcpc"
...
cs.store(group="agent", name="srcpc", node=SRCPCAgentConfig)
...
class SRCPCAgent:
    ...
    def __init__(self, **kwargs: tp.Any):
        ...
        self.icm = ICM1(self.obs_dim, self.action_dim, cfg.hidden_dim,
        ...
        self.icm.train()
        self.quantile = 0.9
        self.update_actor_target = 10
        
        self.domain = cfg.domain
        self.seed = cfg.seed
        self.work_dir = os.path.join(str(self.domain)+'_'+str(cfg.obs_type)+'_pretrain', str(self.cfg.feature_learner), str(self.seed))
    ...
    def infer_meta_from_obs_and_rewards(self, obs: torch.Tensor, reward: torch.Tensor, next_obs: torch.Tensor):
        idx = torch.where(reward >= torch.quantile(reward, 0.9))[0]
        obs = obs[idx]
        reward = reward[idx]
    ...
    def update_sf(
        ...
        # calculate icm_loss
        icm_loss, metrics = self.calculate_icm_loss(origin_obs, obs, z, action, next_obs, metrics, step)

        # walker:1:10, cheetah:1:1
        loss = sf_loss + icm_loss
    ...
    def update_actor(self, obs: torch.Tensor, z: torch.Tensor, old_action: torch.Tensor, step: int) -> tp.Dict[str, float]:
        ...
        if self.cfg.boltzmann:
            ...
            action = dist.rsample()
            with torch.no_grad():
                dist_target = self.actor_target(obs, z)
                action_target = dist_target.rsample()
        else:
            stddev = utils.schedule(self.cfg.stddev_schedule, step)
            action = self.actor(obs, z)
            with torch.no_grad():
                action_target = self.actor_target(obs, z)
    ...
    def aug_and_encode_salient(self, obs, z, action):
        obs = self.aug(obs)
        obs_grad = fb_compute_attribution(self.encoder, self.successor_net, obs, z, action.detach())
    ...
    def calculate_icm_loss(self, origin_obs, obs, z, action, next_obs, metrics, step):
        metrics = dict()
    ...
    def update(self, replay_loader: ReplayBuffer, step: int) -> tp.Dict[str, float]:
        ...
        for _ in range(self.cfg.num_sf_updates):
            ...
            metrics.update(self.update_sf(obs=obs, origin_obs=origin_obs, action=action, discount=discount, next_obs=next_obs.detach(), future_obs=future_obs.detach(), z=z, step=step))
        ...
        if step % self.update_actor_target == 0:
            # update critic target
            utils.soft_update_params(self.actor, self.actor_target, self.cfg.sf_target_tau)
    ...
    def save_image(self, obs, z, action, step, prefix="original"):
        ...
        critic_obs_grad = fb_compute_attribution(self.encoder, self.successor_net, obs, z, action.detach())
