...
class Actor(nn.Module):
    def __init__(self, obs_dim, z_dim, action_dim, feature_dim, hidden_dim,
        ...
        self.embed = mlp(self.obs_dim, hidden_dim, "ntanh", feature_dim)
        
        if self.preprocess:
            self.obs_net = mlp(feature_dim, hidden_dim, "ntanh", feature_dim, "irelu")
            self.obs_z_net = mlp(feature_dim + self.z_dim, hidden_dim, "ntanh", feature_dim, "irelu")
            if not add_trunk:
            ...
            self.trunk = mlp(feature_dim + self.z_dim, hidden_dim, "ntanh",
                             hidden_dim, "irelu",
                             hidden_dim, "irelu")
            feature_dim = hidden_dim
...
class ForwardMap(nn.Module):
    ...
    def __init__(self, obs_dim, z_dim, latent_a_dim, feature_dim, hidden_dim,
                 preprocess=False, add_trunk=True) -> None:
        super().__init__()
        self.obs_dim = obs_dim
        self.z_dim = z_dim
        ...
        if self.preprocess:
            self.obs_action_net = mlp(feature_dim + self.latent_a_dim, hidden_dim, "ntanh", feature_dim, "irelu")
            self.obs_z_net = mlp(feature_dim + self.z_dim, hidden_dim, "ntanh", feature_dim, "irelu")
            if not add_trunk:
                self.trunk: nn.Module = nn.Identity()
                feature_dim = 2 * feature_dim
            ...
            self.trunk = mlp(feature_dim + self.z_dim + self.latent_a_dim, hidden_dim, "ntanh",
                             hidden_dim, "irelu",
                             hidden_dim, "irelu")
            feature_dim = hidden_dim
    ...
    def forward(self, obs, z, action, act_tok=None):
        assert z.shape[-1] == self.z_dim
