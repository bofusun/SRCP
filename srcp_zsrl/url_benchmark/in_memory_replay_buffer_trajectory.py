...
class ReplayBuffer:
    ...
    def sample(self, batch_size, custom_reward: tp.Optional[tp.Any] = None, with_physics: bool = False) -> EpisodeBatch:
        ...
        # print("tttttt", self._storage['observation'][ep_idx, step_idx-1].shape)
        obs = np.stack([self.get_obs("observation", ep_idx, step_idx+i - 1) for i in range(self._multistep)], axis=1)
        # action = self._storage['action'][ep_idx, step_idx]
        action = np.stack([self._storage['action'][ep_idx, step_idx+i] for i in range(self._multistep)], axis=1)
        # next_obs = self.get_obs("observation", ep_idx, step_idx)
        next_obs = np.stack([self.get_obs("observation", ep_idx, step_idx+i) for i in range(self._multistep)], axis=1)
