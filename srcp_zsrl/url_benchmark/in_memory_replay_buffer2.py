#########################################################################
# 额外batch中增加一个轨迹，匹配train_offline1与fb_ddpg8,9
#########################################################################
import logging
import typing as tp
import dataclasses
...
class EpisodeBatch(tp.Generic[T]):
    ...
    def unpack(self) -> tp.Tuple[T, T, T, T, T, T]:
        ...
        """
        return (self.obs, self.action, self.reward, self.discount, self.next_obs, self.trajectory_obs)
...
class ReplayBuffer:
    ...
    def get_trajectory_obs(self, storage_key, ep_idx, step_idx):
        if self._frame_stack is None:
            return self._storage[storage_key][ep_idx, step_idx:step_idx+self._trajectory_num]
        else:
            rets = []
            counts = np.random.choice(self._trajectory_num, size=5, replace=False)
            for j in counts:
                stacked_obs = []
                for i in range(self._frame_stack):
                    # print("step_idx", step_idx)
                    # print("step_idx+j", step_idx+j)
                    stacked_obs.append(self._storage[storage_key][ep_idx, np.maximum(0, step_idx+j - (self._frame_stack - i - 1))])
                # print("stacked_obs", np.array(stacked_obs).shape)
                rets.append(np.concatenate(stacked_obs, axis=1))
            rets = np.array(rets).transpose(1, 0, 2, 3, 4)
            # print("rets", np.array(rets).shape)
            return rets
    ...
    def sample(self, batch_size, custom_reward: tp.Optional[tp.Any] = None, with_physics: bool = False) -> EpisodeBatch:
        ...
        step_idx = np.random.randint(0, eps_lengths-self._trajectory_num-1) + 1
        ...
        trajectory_obs = self.get_trajectory_obs("observation", ep_idx, step_idx-1)
        ...
        return EpisodeBatch(obs=obs, action=action, reward=reward, discount=discount,
                            next_obs=next_obs, trajectory_obs=trajectory_obs, future_obs=future_obs, meta=meta, **additional)
