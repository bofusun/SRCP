...
from url_benchmark.in_memory_replay_buffer4 import ReplayBuffer
...
class Workspace:
    def __init__(self, cfg: Config) -> None:
        ...
        self._checkpoint_filepath = Path("/data/sjb/HILP-master/hilp_zsrl/exp_local/EXP/sd10...20250711173301.Ewalker_run.Msf24_offline/models/latest.pt")
        if self._checkpoint_filepath.exists():
            self.load_checkpoint(self._checkpoint_filepath, exclude=["replay_loader"])
    ...
    def load_checkpoint(self, fp: tp.Union[Path, str], only: tp.Optional[tp.Sequence[str]] = None, exclude: tp.Sequence[str] = (), num_episodes=None, use_pixels=False) -> None:
        ...
        print(f"eeeeeeeeeeeeeeeeeeeeeeeeeeee")
        if use_pixels:
        ...
        for x in exclude:
            payload.pop(x, None)
        print(f"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb")
        for name, val in payload.items():
            ...
            if name == "agent":
                ...
                # self.replay_loader._max_episodes = len(val._storage["discount"])
                # val.sample = self.replay_loader.sample
                # self.replay_loader = val
                # 获取 val 的所有属性字典
                val_attributes = vars(val)  # 或者 val.__dict__
                # 逐个赋值给 self.replay_loader，排除 'sample' 方法
                for key, value in val_attributes.items():
                    if key != 'sample':  # 排除 'sample' 方法
                        setattr(self.replay_loader, key, value)
                # self.replay_loader._current_episode.clear()
                # self.replay_loader._max_episodes = len(val._storage["discount"])
                # self.replay_loader._storage = val._storage
            else:
    ...
    def finalize(self) -> None:
        ...
        for name in domain_tasks[self.domain]:
            ...
            info = self.eval(final_eval=True)
